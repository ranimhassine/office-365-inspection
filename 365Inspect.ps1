...
Function HTML-Report {
    ...
    function Parse-Template {
        $template = (Get-Content ".\365InspectConsultimTemplate.html") -join "`n"
    ...
    ForEach ($finding in $sortedFindings) {
        ...
        If ($null -NE $finding.AffectedObjects) {
            ...
            If ($finding.Impact -eq 'Critical') {
                $criticalCount += 1
                $htmlImpact = '<span class="badge badge-Critical">Critical</span>'
            ...
            ElseIf ($finding.Impact -eq 'High') {
                $highCount += 1
                $htmlImpact = '<span class="badge badge-High">High</span>'
                $short_finding_html = $short_finding_html.Replace("{{IMPACT}}", $htmlImpact)
                $long_finding_html = $long_finding_html.Replace("{{IMPACT}}", $htmlImpact)
            }
            ElseIf ($finding.Impact -eq 'Medium') {
                $htmlImpact = '<span class="badge badge-Medium">Medium</span>'
                $short_finding_html = $short_finding_html.Replace("{{IMPACT}}", $htmlImpact)
                $long_finding_html = $long_finding_html.Replace("{{IMPACT}}", $htmlImpact)
            }
            ElseIf ($finding.Impact -eq 'Low') {
                $htmlImpact = '<span class="badge badge-Low">Low</span>'
                $short_finding_html = $short_finding_html.Replace("{{IMPACT}}", $htmlImpact)
                $long_finding_html = $long_finding_html.Replace("{{IMPACT}}", $htmlImpact)
            }
            Else {
                $htmlImpact = '<span class="badge badge-Informational">' + $finding.Impact + '</span>'
                $short_finding_html = $short_finding_html.Replace("{{IMPACT}}", $htmlImpact)
                $long_finding_html = $long_finding_html.Replace("{{IMPACT}}", $htmlImpact)
            }
    ...
    $chart_template_html = $chart_template_html.Replace("{{TOTAL_COUNT}}", $findings_count.ToString())
